          ...
          {tabs.map((group, index) => (
            <ul key={index} className='w-full border-b-[1px] border-black/5'>
              {group.map((tab, tabIndex) => (
                <li
                  key={tabIndex}
                  className={`w-full flex items-center justify-between py-3 my-2 pl-6 pr-4 relative before:rounded-xl before:absolute before:w-1 before:left-0   before:h-full before:duration-500 ${routing.some(path => tab.link && path === tab.link) ? 'before:bg-red-500' : 'before:bg-white'}`}
                >
